...
pub type GBMDevice = *mut ffi::Struct_gbm_device;
pub type GBMBufferObject = *mut ffi::Struct_gbm_bo;
pub type GBMSurface = *mut ffi::Struct_gbm_surface;
...
pub fn device_get_backend_name<'a>(gbm: &GBMDevice) -> &'a str {
    ...
    let bytes: &'a [u8] = unsafe {
        let c_str: &'a CStr = CStr::from_ptr(ffi::gbm_device_get_backend_name(*gbm));
        c_str.to_bytes()
    };
...
pub fn device_is_format_supported(gbm: &GBMDevice, format: u32,
                                  usage: u32) -> bool {
    match unsafe { ffi::gbm_device_is_format_supported(*gbm, format, usage) } {
        0 => false,
        _ => true
    }
}
...
pub fn create_device(fd: i32) -> Option<GBMDevice> {
    match unsafe { ffi::gbm_create_device(fd) } as usize {
        0 => None,
...
pub fn bo_create(gbm: &GBMDevice, width: u32, height: u32,
    ...
    match unsafe {
        ffi::gbm_bo_create(*gbm, width, height, format, flags)
    } as usize {
        0 => None,
...
pub fn surface_create(gbm: &GBMDevice, width: u32, height: u32,
    ...
    assert!(*gbm as usize != 0);
    match unsafe {
        ffi::gbm_surface_create(*gbm, width, height, format, flags)
    } as usize {
        0 => None,
...
pub fn surface_needs_lock_front_buffer(surface: &GBMSurface) -> i32 {
    ...
    unsafe {
        ffi::gbm_surface_needs_lock_front_buffer(*surface)
    }
}
...
pub fn surface_lock_front_buffer(surface: &GBMSurface) -> Option<GBMBufferObject> {
    assert!(*surface as usize != 0);
    match unsafe {
        ffi::gbm_surface_lock_front_buffer(*surface)
    } as usize {
        0 => None,
        x => Some(x as GBMBufferObject)
    }
...
pub fn surface_release_buffer(surface: &GBMSurface,
    ...
    assert!(bo as usize != 0);
    unsafe {
        ffi::gbm_surface_release_buffer(*surface, bo);
    }
}

pub fn surface_has_free_buffers(surface: &GBMSurface) -> bool {
    assert!(*surface as usize != 0);
    match unsafe { ffi::gbm_surface_has_free_buffers(*surface) } {
        0 => false,
        _ => true
    }
}
...
pub fn surface_destroy(surface: GBMSurface) -> () {
    assert!(surface as usize != 0);
    unsafe {
        ffi::gbm_surface_destroy(surface);
    }
}
